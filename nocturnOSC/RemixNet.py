...
class OSCServer:
    ...
    def try_server(self):
        if not self.udpServer.is_bound():
            self.parent.log('trying to bind')
            self.udpServer.bind()
    ...
    def processIncomingUDP(self):
        ...
        * The Ableton Live Python environment, although it includes the thread
        ...
        if self.retry % 50 == 0:
            ...
            if not self.udpClient.outgoing():
                self.udpClient.setDst(self.parent.remote_ip)
                self.udpClient.setDstPort(self.parent.send_port)
        ...
        if self.udpServer.is_bound():
            ...
            if self.retry % 50 == 0:
                self.parent.c_instance.show_message('Retrying oscServer: ' + str(self.retry))
                
                self.udpServer.set_network(self.parent.local_ip, self.parent.receive_port)
                self.try_server()
...
class UDPServer:
    ...
    def bind(self):
        ...
        # self.parent.log(str(self.addr))
        
        try:
            self.UDPSock.bind(self.addr)
            self.UDPSock.setblocking(0)
            self.bound = 1
            self.parent.c_instance.show_message('listening on %s:%s' % (self.src,self.srcPort))
        except:
            self.bound = 0
            self.parent.c_instance.show_message('ERROR: Could not bind to incoming ip/port, check your NocturnOSC network settings')
